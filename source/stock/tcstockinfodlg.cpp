...
tcStockInfoDialog::tcStockInfoDialog(QWidget *pParent, tcMarket *pMarket, bool pAppendMode)
	: QDialog(pParent)
{
	mMarket = pMarket;
...
void tcStockInfoDialog::DoOk()
{
	if (mAppendMode) {
		...
		if (mMarket->GetStockByCode(edt1->text()) != NULL) {
...
#ifdef WIN32
	#include "moc_tcstockinfodlg.cpp"
#endif
